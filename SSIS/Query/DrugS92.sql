...
DECLARE @DBIdDrugDaroshafa92 INT;
...
VALUES
(N'DrugDaroshafa92' -- Name - nvarchar(50)
    );
...
SELECT @DBIdDrugDaroshafa92 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'DrugDaroshafa92';
...
SELECT ISNULL(UserName,Lname),
       ...
       Users.Id,
       N'کاربر'
FROM DrugDaroshafa92.dbo.Users
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM DrugDaroshafa92.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT DrugDaroshafa92.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrugDaroshafa92,               -- DBId - int
       DrugDaroshafa92.dbo.Bimeh.BimehCode -- OldId - int
FROM DrugDaroshafa92.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrugDaroshafa92,
       S.CodeSazeMan
FROM DrugDaroshafa92.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM DrugDaroshafa92.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrugDaroshafa92;
...
--        WHERE DD2.[Name] = N'DrugDaroshafa92'
--    ) AS InsuranceMCode
--WHERE @DBIdDrugDaroshafa92 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM DrugDaroshafa92.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrugDaroshafa92,
       T.CodeTakh
FROM DrugDaroshafa92.dbo.Takhasos AS T
    INNER JOIN DrugDaroshafa92.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrugDaroshafa92 = DPL.DBId;
...
--WHERE DPL.DBId = @DBIdDrugDaroshafa92
--      AND DPL.OldId = 0;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM DrugDaroshafa92.dbo.Dr AS D
    LEFT JOIN DrugDaroshafa92.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN DrugDaroshafa92.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrugDaroshafa92 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrugDaroshafa92 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT DrugDaroshafa92.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrugDaroshafa92,
       DrugDaroshafa92.dbo.DrugGroup.CodeGroup
FROM DrugDaroshafa92.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPG.Id, DPG2.Id),
       DrugDaroshafa92.dbo.Kala.Code,
       ISNULL(DrugDaroshafa92.dbo.Kala.FaName, DrugDaroshafa92.dbo.Kala.Code),
       @DBIdDrugDaroshafa92,
       DrugDaroshafa92.dbo.Kala.Code
FROM DrugDaroshafa92.dbo.Kala
    LEFT JOIN dbo.DimProductGroup AS DPG
        ON DPG.OldId = DrugDaroshafa92.dbo.Kala.CodeGroup
           AND DPG.DBId = @DBIdDrugDaroshafa92
...
--               DBId = @DBIdDrugDaroshafa92
--        FROM dbo.DimProduct AS DP
--        WHERE DP.DBId = @DBIdDrugDaroshafa92
--    ) AS maxNumberT
...
FROM DrugDaroshafa92.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM DrugDaroshafa92.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM DrugDaroshafa92.dbo.FacHeder AS FH
    ...
    LEFT JOIN DrugDaroshafa92.dbo.Dr AS D
    ...
    LEFT JOIN dbo.DimPhysician AS DP
        ON DP.Mama = T2.Mama
           ...
           AND DP.DBId = @DBIdDrugDaroshafa92
    ...
    INNER JOIN dbo.DimFinancialYear AS DFY
        ON DFY.DBId = @DBIdDrugDaroshafa92
    LEFT JOIN DrugDaroshafa92.dbo.Sazeman AS S
        ON S.CodeSazeMan = FH.CodeSazeMan
    LEFT JOIN dbo.DimInsurance AS DI
        ON DI.OldId = S.CodeSazeMan
           AND DI.DBId = @DBIdDrugDaroshafa92
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM DrugDaroshafa92.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM DrugDaroshafa92.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN DrugDaroshafa92.dbo.FacRadif AS FR
...
WHERE DP2.OldId = 0
      ...
      AND FH.DBId = @DBIdDrugDaroshafa92;
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrugDaroshafa92;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrugDaroshafa92,
       K.KheyrieCode
FROM DrugDaroshafa92.dbo.Kheyrie AS K
...
--           AND DOT.DBId = @DBIdDrugDaroshafa92
--WHERE DO.OldId = 1;
...
FROM DrugDaroshafa92.dbo.FacHeder AS FH
    ...
    INNER JOIN DrugDaroshafa92.dbo.Sandogh AS S
...
WHERE FH.State IN ( 0, 10 )
      ...
      AND DOTUnk.OldId = 0
      AND FH2.DBId = @DBIdDrugDaroshafa92
      AND S.State = 1
