...
SELECT @DBIdDrug93 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'Drug93';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM Drug93.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT Drug93.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrug93,               -- DBId - int
       Drug93.dbo.Bimeh.BimehCode -- OldId - int
FROM Drug93.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrug93,
       S.CodeSazeMan
FROM Drug93.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM Drug93.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrug93;
...
--        WHERE DD2.[Name] = N'Drug93'
--    ) AS InsuranceMCode
--WHERE @DBIdDrug93 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM Drug93.dbo.Takhasos2 AS T;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM Drug93.dbo.Dr AS D
    LEFT JOIN Drug93.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN Drug93.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrug93 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrug93 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT Drug93.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrug93,
       Drug93.dbo.DrugGroup.CodeGroup
FROM Drug93.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPF.Id, DPF2.Id),
       Drug93.dbo.Kala.Code,
       ISNULL(Drug93.dbo.Kala.FaName, Drug93.dbo.Kala.Code),
       @DBIdDrug93,
       Drug93.dbo.Kala.Code
FROM Drug93.dbo.Kala
    LEFT JOIN dbo.DimProductForm AS DPF
        ON DPF.OldId = Drug93.dbo.Kala.CodeGroup
           AND DPF.DBId = @DBIdDrug93
...
FROM Drug93.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM Drug93.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM Drug93.dbo.FacHeder AS FH
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM Drug93.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM Drug93.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN Drug93.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrug93;
...
FROM Drug93.dbo.FacHeder AS FH
    ...
    INNER JOIN Drug93.dbo.Sandogh AS S
