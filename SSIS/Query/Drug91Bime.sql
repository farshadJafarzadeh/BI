...
SELECT @DBIdDrug91Bime = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'Drug91Bime';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM Drug91Bime.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT Drug91Bime.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrug91Bime,               -- DBId - int
       Drug91Bime.dbo.Bimeh.BimehCode -- OldId - int
FROM Drug91Bime.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrug91Bime,
       S.CodeSazeMan
FROM Drug91Bime.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM Drug91Bime.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrug91Bime;
...
--        WHERE DD2.[Name] = N'Drug91Bime'
--    ) AS InsuranceMCode
--WHERE @DBIdDrug91Bime = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM Drug91Bime.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrug91Bime,
       T.CodeTakh
FROM Drug91Bime.dbo.Takhasos AS T
    INNER JOIN Drug91Bime.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrug91Bime = DPL.DBId;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM Drug91Bime.dbo.Dr AS D
    LEFT JOIN Drug91Bime.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN Drug91Bime.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrug91Bime = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrug91Bime = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT Drug91Bime.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrug91Bime,
       Drug91Bime.dbo.DrugGroup.CodeGroup
FROM Drug91Bime.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPF.Id, DPF2.Id),
       Drug91Bime.dbo.Kala.Code,
       ISNULL(Drug91Bime.dbo.Kala.FaName, Drug91Bime.dbo.Kala.Code),
       @DBIdDrug91Bime,
       Drug91Bime.dbo.Kala.Code
FROM Drug91Bime.dbo.Kala
    LEFT JOIN dbo.DimProductForm AS DPF
        ON DPF.OldId = Drug91Bime.dbo.Kala.CodeGroup
           AND DPF.DBId = @DBIdDrug91Bime
...
FROM Drug91Bime.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM Drug91Bime.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM Drug91Bime.dbo.FacHeder AS FH
    ...
    LEFT JOIN Drug91Bime.dbo.Dr AS D
    ...
    INNER JOIN dbo.DimFinancialYear AS DFY
        ON DFY.DBId = @DBIdDrug91Bime
    LEFT JOIN Drug91Bime.dbo.Sazeman AS S
        ON S.CodeSazeMan = FH.CodeSazeMan
    LEFT JOIN dbo.DimInsurance AS DI
        ON DI.OldId = S.CodeSazeMan
           AND DI.DBId = @DBIdDrug91Bime
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM Drug91Bime.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM Drug91Bime.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN Drug91Bime.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrug91Bime;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrug91Bime,
       K.KheyrieCode
FROM Drug91Bime.dbo.Kheyrie AS K
...
FROM Drug91Bime.dbo.FacHeder AS FH
    ...
    INNER JOIN Drug91Bime.dbo.Sandogh AS S
