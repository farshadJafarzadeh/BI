...
SELECT @DBIdDrugShafa94 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'DrugShafa94';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM DrugShafa94.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT DrugShafa94.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrugShafa94,               -- DBId - int
       DrugShafa94.dbo.Bimeh.BimehCode -- OldId - int
FROM DrugShafa94.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrugShafa94,
       S.CodeSazeMan
FROM DrugShafa94.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM DrugShafa94.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrugShafa94;
...
--        WHERE DD2.[Name] = N'DrugShafa94'
--    ) AS InsuranceMCode
--WHERE @DBIdDrugShafa94 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM DrugShafa94.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrugShafa94,
       T.CodeTakh
FROM DrugShafa94.dbo.Takhasos AS T
    INNER JOIN DrugShafa94.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrugShafa94 = DPL.DBId;
...
--WHERE DPL.DBId = @DBIdDrugShafa94
--      AND DPL.OldId = 0;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM DrugShafa94.dbo.Dr AS D
    LEFT JOIN DrugShafa94.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN DrugShafa94.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrugShafa94 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrugShafa94 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT DrugShafa94.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrugShafa94,
       DrugShafa94.dbo.DrugGroup.CodeGroup
FROM DrugShafa94.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
FROM DrugShafa94.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM DrugShafa94.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM DrugShafa94.dbo.FacHeder AS FH
    ...
    LEFT JOIN DrugShafa94.dbo.Dr AS D
    ...
    INNER JOIN dbo.DimFinancialYear AS DFY
        ON DFY.DBId = @DBIdDrugShafa94
    LEFT JOIN DrugShafa94.dbo.Sazeman AS S
        ON S.CodeSazeMan = FH.CodeSazeMan
    LEFT JOIN dbo.DimInsurance AS DI
        ON DI.OldId = S.CodeSazeMan
           AND DI.DBId = @DBIdDrugShafa94
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM DrugShafa94.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM DrugShafa94.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN DrugShafa94.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrugShafa94;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrugShafa94,
       K.KheyrieCode
FROM DrugShafa94.dbo.Kheyrie AS K
...
FROM DrugShafa94.dbo.FacHeder AS FH
    ...
    INNER JOIN DrugShafa94.dbo.Sandogh AS S
