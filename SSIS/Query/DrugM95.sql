...
SELECT @DBIdDrugMon95 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'DrugMon95';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM DrugMon95.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT DrugMon95.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrugMon95,               -- DBId - int
       DrugMon95.dbo.Bimeh.BimehCode -- OldId - int
FROM DrugMon95.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrugMon95,
       S.CodeSazeMan
FROM DrugMon95.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM DrugMon95.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrugMon95;
...
--        WHERE DD2.[Name] = N'DrugMon95'
--    ) AS InsuranceMCode
--WHERE @DBIdDrugMon95 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM DrugMon95.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrugMon95,
       T.CodeTakh
FROM DrugMon95.dbo.Takhasos AS T
    INNER JOIN DrugMon95.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrugMon95 = DPL.DBId;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM DrugMon95.dbo.Dr AS D
    LEFT JOIN DrugMon95.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN DrugMon95.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrugMon95 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrugMon95 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT DrugMon95.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrugMon95,
       DrugMon95.dbo.DrugGroup.CodeGroup
FROM DrugMon95.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPG.Id, DPG2.Id),
       DrugMon95.dbo.Kala.Code,
       ISNULL(DrugMon95.dbo.Kala.FaName, DrugMon95.dbo.Kala.Code),
       @DBIdDrugMon95,
       DrugMon95.dbo.Kala.Code
FROM DrugMon95.dbo.Kala
    LEFT JOIN dbo.DimProductGroup AS DPG
        ON DPG.OldId = DrugMon95.dbo.Kala.CodeGroup
           AND DPG.DBId = @DBIdDrugMon95
...
FROM DrugMon95.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM DrugMon95.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM DrugMon95.dbo.FacHeder AS FH
    ...
    LEFT JOIN DrugMon95.dbo.Dr AS D
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM DrugMon95.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM DrugMon95.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN DrugMon95.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrugMon95;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrugMon95,
       K.KheyrieCode
FROM DrugMon95.dbo.Kheyrie AS K
...
FROM DrugMon95.dbo.FacHeder AS FH
    ...
    INNER JOIN DrugMon95.dbo.Sandogh AS S
