...
DECLARE @DBIdDrugDaroshafa9107 INT;
...
VALUES
(N'DrugDaroshafa9107' -- Name - nvarchar(50)
    );
...
SELECT @DBIdDrugDaroshafa9107 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'DrugDaroshafa9107';
...
SELECT ISNULL(UserName, Lname),
       ...
       Lname,
       DimOwner.Id,
       @DBIdDrugDaroshafa9107,
       Users.Id,
       N'کاربر'
FROM DrugDaroshafa9107.dbo.Users
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM DrugDaroshafa9107.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT DrugDaroshafa9107.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrugDaroshafa9107,               -- DBId - int
       DrugDaroshafa9107.dbo.Bimeh.BimehCode -- OldId - int
FROM DrugDaroshafa9107.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrugDaroshafa9107,
       S.CodeSazeMan
FROM DrugDaroshafa9107.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM DrugDaroshafa9107.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrugDaroshafa9107;
...
--        WHERE DD2.[Name] = N'DrugDaroshafa9107'
--    ) AS InsuranceMCode
--WHERE @DBIdDrugDaroshafa9107 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM DrugDaroshafa9107.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrugDaroshafa9107,
       T.CodeTakh
FROM DrugDaroshafa9107.dbo.Takhasos AS T
    INNER JOIN DrugDaroshafa9107.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrugDaroshafa9107 = DPL.DBId;
...
--WHERE DPL.DBId = @DBIdDrugDaroshafa9107
--      AND DPL.OldId = 0;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM DrugDaroshafa9107.dbo.Dr AS D
    LEFT JOIN DrugDaroshafa9107.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN DrugDaroshafa9107.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrugDaroshafa9107 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrugDaroshafa9107 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT DrugDaroshafa9107.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrugDaroshafa9107,
       DrugDaroshafa9107.dbo.DrugGroup.CodeGroup
FROM DrugDaroshafa9107.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPF.Id, DPF2.Id),
       DrugDaroshafa9107.dbo.Kala.Code,
       ISNULL(DrugDaroshafa9107.dbo.Kala.FaName, DrugDaroshafa9107.dbo.Kala.Code),
       @DBIdDrugDaroshafa9107,
       DrugDaroshafa9107.dbo.Kala.Code
FROM DrugDaroshafa9107.dbo.Kala
    LEFT JOIN dbo.DimProductForm AS DPF
        ON DPF.OldId = DrugDaroshafa9107.dbo.Kala.CodeGroup
           AND DPF.DBId = @DBIdDrugDaroshafa9107
...
--               DBId = @DBIdDrugDaroshafa9107
--        FROM dbo.DimProduct AS DP
--        WHERE DP.DBId = @DBIdDrugDaroshafa9107
--    ) AS maxNumberT
...
FROM DrugDaroshafa9107.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM DrugDaroshafa9107.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM DrugDaroshafa9107.dbo.FacHeder AS FH
    ...
    LEFT JOIN DrugDaroshafa9107.dbo.Dr AS D
    ...
    LEFT JOIN dbo.DimPhysician AS DP
        ON DP.Mama = T2.Mama
           ...
           AND DP.DBId = @DBIdDrugDaroshafa9107
    ...
    INNER JOIN dbo.DimFinancialYear AS DFY
        ON DFY.DBId = @DBIdDrugDaroshafa9107
    LEFT JOIN DrugDaroshafa9107.dbo.Sazeman AS S
        ON S.CodeSazeMan = FH.CodeSazeMan
    LEFT JOIN dbo.DimInsurance AS DI
        ON DI.OldId = S.CodeSazeMan
           AND DI.DBId = @DBIdDrugDaroshafa9107
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM DrugDaroshafa9107.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM DrugDaroshafa9107.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN DrugDaroshafa9107.dbo.FacRadif AS FR
...
WHERE DP2.OldId = 0
      ...
      AND FH.DBId = @DBIdDrugDaroshafa9107;
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrugDaroshafa9107;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrugDaroshafa9107,
       K.KheyrieCode
FROM DrugDaroshafa9107.dbo.Kheyrie AS K
...
--           AND DOT.DBId = @DBIdDrugDaroshafa9107
--WHERE DO.OldId = 1;
...
FROM DrugDaroshafa9107.dbo.FacHeder AS FH
    ...
    INNER JOIN DrugDaroshafa9107.dbo.Sandogh AS S
...
WHERE FH.State IN ( 0, 10 )
      ...
      AND DOTUnk.OldId = 0
      AND FH2.DBId = @DBIdDrugDaroshafa9107
