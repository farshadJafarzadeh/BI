...
SELECT @DBIdDrugMon93 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'DrugMon93';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM DrugMon93.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT DrugMon93.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrugMon93,               -- DBId - int
       DrugMon93.dbo.Bimeh.BimehCode -- OldId - int
FROM DrugMon93.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrugMon93,
       S.CodeSazeMan
FROM DrugMon93.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM DrugMon93.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrugMon93;
...
--        WHERE DD2.[Name] = N'DrugMon93'
--    ) AS InsuranceMCode
--WHERE @DBIdDrugMon93 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM DrugMon93.dbo.Takhasos2 AS T;
...
SELECT DPL.Id,
       ...
       @DBIdDrugMon93,
       T.CodeTakh
FROM DrugMon93.dbo.Takhasos AS T
    INNER JOIN DrugMon93.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    INNER JOIN dbo.DimPhysicianLevel AS DPL
        ON DPL.OldId = T2.CodeTakh2
           AND @DBIdDrugMon93 = DPL.DBId;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM DrugMon93.dbo.Dr AS D
    LEFT JOIN DrugMon93.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN DrugMon93.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrugMon93 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrugMon93 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT DrugMon93.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrugMon93,
       DrugMon93.dbo.DrugGroup.CodeGroup
FROM DrugMon93.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPG.Id, DPG2.Id),
       DrugMon93.dbo.Kala.Code,
       ISNULL(DrugMon93.dbo.Kala.FaName, DrugMon93.dbo.Kala.Code),
       @DBIdDrugMon93,
       DrugMon93.dbo.Kala.Code
FROM DrugMon93.dbo.Kala
    LEFT JOIN dbo.DimProductGroup AS DPG
        ON DPG.OldId = DrugMon93.dbo.Kala.CodeGroup
           AND DPG.DBId = @DBIdDrugMon93
...
FROM DrugMon93.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM DrugMon93.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM DrugMon93.dbo.FacHeder AS FH
    ...
    LEFT JOIN DrugMon93.dbo.Dr AS D
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM DrugMon93.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM DrugMon93.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN DrugMon93.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrugMon93;
...
SELECT K.KheyrieCode,
       ...
       @DBIdDrugMon93,
       K.KheyrieCode
FROM DrugMon93.dbo.Kheyrie AS K
...
FROM DrugMon93.dbo.FacHeder AS FH
    ...
    INNER JOIN DrugMon93.dbo.Sandogh AS S
