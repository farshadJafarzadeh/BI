...
SELECT @DBIdDrug90 = DD.Id
FROM dbo.DimDB AS DD
WHERE DD.Name = N'Drug90';
...
SELECT MF.MasolFani,
       ...
       MF.CodeMasolFani,
       N'مسئول فنی'
FROM Drug90.dbo.MasolFani AS MF
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1;
...
SELECT Drug90.dbo.Bimeh.Bimeh,    -- Title - nvarchar(600)
       @DBIdDrug90,               -- DBId - int
       Drug90.dbo.Bimeh.BimehCode -- OldId - int
FROM Drug90.dbo.Bimeh;
...
SELECT dbo.DimInsuranceGroup.Id,
       ...
       @DBIdDrug90,
       S.CodeSazeMan
FROM Drug90.dbo.Sazeman AS S
    INNER JOIN
    (
        SELECT SIB.CodeSazeman,
               SIB.BimehCode
        FROM Drug90.dbo.SazemanInBimeh AS SIB
            INNER JOIN
            (
                SELECT DISTINCT
                       SIB.CodeSazeman,
    ...
    INNER JOIN dbo.DimInsuranceGroup
        ON SIB.BimehCode = DimInsuranceGroup.OldId
           AND DimInsuranceGroup.DBId = @DBIdDrug90;
...
--        WHERE DD2.[Name] = N'Drug90'
--    ) AS InsuranceMCode
--WHERE @DBIdDrug90 = DIG.DBId
...
SELECT T.Takhasos2,
       ...
       T.CodeTakh2,
       T.Mama
FROM Drug90.dbo.Takhasos2 AS T;
...
SELECT DISTINCT
       ...
       D.CodeDr,
       T2.Mama
FROM Drug90.dbo.Dr AS D
    LEFT JOIN Drug90.dbo.Takhasos AS T
        ON T.CodeTakh = D.CodeTakh
    LEFT JOIN Drug90.dbo.Takhasos2 AS T2
        ON T2.CodeTakh2 = T.CodeTakh2
    LEFT JOIN dbo.DimPhysicianSpeciality AS DPS
        ON DPS.OldId = T.CodeTakh
           AND @DBIdDrug90 = DPS.DBId
    INNER JOIN dbo.DimPhysicianSpeciality AS DPS2
        ON DPS2.OldId = 0
           AND @DBIdDrug90 = DPS2.DBId
ORDER BY D.CodeDr;
...
SELECT Drug90.dbo.DrugGroup.DrugGroup,
       dbo.DimOwner.Id,
       @DBIdDrug90,
       Drug90.dbo.DrugGroup.CodeGroup
FROM Drug90.dbo.DrugGroup
    CROSS JOIN dbo.DimOwner
WHERE dbo.DimOwner.OldId = 1
...
SELECT ISNULL(DPF.Id, DPF2.Id),
       Drug90.dbo.Kala.Code,
       ISNULL(Drug90.dbo.Kala.FaName, Drug90.dbo.Kala.Code),
       @DBIdDrug90,
       Drug90.dbo.Kala.Code
FROM Drug90.dbo.Kala
    LEFT JOIN dbo.DimProductForm AS DPF
        ON DPF.OldId = Drug90.dbo.Kala.CodeGroup
           AND DPF.DBId = @DBIdDrug90
...
FROM Drug90.dbo.FacHeder AS FH
    CROSS JOIN
    (
        SELECT MaxSystemNumber = MAX(Sh_Noskhe)
        FROM Drug90.dbo.FacHeder
        WHERE State IN ( 0, 10 )
    ) AS FHMax
...
FROM Drug90.dbo.FacHeder AS FH
    ...
    (
        SELECT S.Sh_Noskhe,
               S.CodeMasolFani
        FROM Drug90.dbo.Sandogh AS S
            INNER JOIN
            (
                SELECT Sh_Noskhe,
                       MaxSandogh = MAX(Sandogh)
                FROM Drug90.dbo.Sandogh
                WHERE State = 1
                GROUP BY Sh_Noskhe
            ) AS S2
...
FROM RPSIDW.dbo.FactHeader AS FH
    INNER JOIN Drug90.dbo.FacRadif AS FR
...
WHERE InsurancePercent > 100
      ...
      OR AddFee < 0
         AND dbo.FactDetail.DBId = @DBIdDrug90;
...
FROM Drug90.dbo.FacHeder AS FH
    ...
    INNER JOIN Drug90.dbo.Sandogh AS S
